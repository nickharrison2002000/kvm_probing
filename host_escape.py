...
class KVMExploiter:
    ...
    def _ioctl(self, cmd: int, arg: Any) -> int:
        ...
        if hasattr(arg, '_obj') and hasattr(arg, '_type_'):
            # This is a byref object, get the address of the underlying object
            arg_value = ctypes.addressof(arg._obj)
        elif hasattr(arg, 'value'):
            # This is a ctypes object with a value attribute
            arg_value = arg.value
        elif isinstance(arg, ctypes.Structure):
            # This is a ctypes Structure, get its address
            arg_value = ctypes.addressof(arg)
        else:
            # Regular integer or buffer
            arg_value = arg
        
        return fcntl.ioctl(self.fd, cmd, arg_value)
